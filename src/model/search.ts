...
const search = (
  ...
  let activeTerms = new Set(Object.entries(query).flatMap(([facet,terms]) => terms.map(t => `${facet}:${t}`)));
  ...
  return {
    ...
    query: { ...query },
    facets: facets || [],
    facetIds: facets.map(f => f.facet_id),
    facetHierarchies,
    terms,
    termIsSelected: (k: string, t: string) => activeTerms.has(`${k}:${t}`),
